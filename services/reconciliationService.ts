...
import { doc, runTransaction, writeBatch, collection, Timestamp } from 'firebase/firestore';
...
export class ReconciliationService {
    ...
    for (const match of matches) {
      try {
        ...
        if (match.needsSplit) {
          ...
          // Fix 6: include projectTransactionIds + autoGenerated in the initial payload so
          // all fields are committed in the single writeBatch inside createTransactionSplit —
          // no subsequent updateTransactionSplit calls needed.
          for (const alloc of match.allocations) {
            const pTx = projectTransactions.find(p => p.id === alloc.projectTxId);
            newSplits.push({
              category: 'Projects & Activities' as const,
              projectId,
              memberId: '',
              amount: alloc.amount,
              ...
              // Extra fields committed atomically in the writeBatch (cast required):
              ...(({ projectTransactionId: alloc.projectTxId, autoGenerated: true }) as any),
            });
          }
          ...
          // Use FinanceService to create the splits (projectTransactionIds already in payload)
  ...
  static async manualMatch(
    ...
    if (needsSplit && !bankTx.isSplit) {
      ...
      // Fix 7: include projectTransactionIds + autoGenerated in the initial payload so
      // all fields are committed in the single writeBatch inside createTransactionSplit.
  ...
  static async unlinkMatch(
    ...
    if (bankTx.isSplit) {
      ...
      if (autoSplits.length > 0) {
        // P1-fix: update reverse indexes first (each is its own runTransaction),
        // then batch all split deletes + parent update into ONE writeBatch so the
        // entire unlink succeeds or fails atomically.
        ...
        const unlinkBatch = writeBatch(db);
        for (const split of autoSplits) {
          unlinkBatch.delete(doc(db, COLLECTIONS.TRANSACTION_SPLITS, split.id));
        }

        // Update parent tx to reflect the deletion of auto-generated splits
        if (remainingIds.length === 0) {
          // All splits were auto-generated — restore parent to its pre-split state
        ...
        const { invalidateFinanceCache } = await import('./financeService');
