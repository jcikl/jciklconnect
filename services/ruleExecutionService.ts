  ...
  private logExecution(execution: RuleExecution): void {
    ...
    if (!isDevMode()) {
      addDoc(collection(db, COLLECTIONS.WORKFLOW_EXECUTIONS), {
        ...
        errorLoggingService.logError(err instanceof Error ? err : new Error(String(err)), { context: 'RuleExecutionService.logExecution', additionalData: { ruleId: execution.ruleId } });
      });
    } else {
