...
export class FinanceService {
  ...
  static async deleteProjectTransaction(transactionId: string): Promise<void> {
    ...
    try {
      ...
      // 1 + 2: Read all linked bank-txs and splits in parallel, then write ALL changes
      // (bank-tx updates + split updates + project-tx delete) in a SINGLE writeBatch so
      // every document succeeds or none does. Previously two separate batch.commit() calls
      // meant a crash between them left bank-txs already updated but splits/project-tx intact.
      const [bankTxsSnapshot, splitsSnapshot] = await Promise.all([
        getDocs(query(
          collection(db, COLLECTIONS.TRANSACTIONS),
      ...
      // fast and never commit partial updates without the corresponding project tx delete.
      ...
      // Single batch covers: bank-tx updates + split updates + project-tx delete (atomic).
      const combinedBatch = writeBatch(db);

      // Collect post-commit side-effects (cannot be batched)
      ...
      for (const docSnap of bankTxsSnapshot.docs) {
        ...
        combinedBatch.update(doc(db, COLLECTIONS.TRANSACTIONS, docSnap.id), {
          projectTransactionIds: newLinkedIds,
          projectTransactionId: newLinkedIds[0] || null,
      ...
      for (const docSnap of splitsSnapshot.docs) {
        ...
        if (split.autoGenerated && newLinkedIds.length === 0) {
          ...
          combinedBatch.update(doc(db, COLLECTIONS.TRANSACTION_SPLITS, docSnap.id), {
            projectTransactionIds: newLinkedIds,
            projectTransactionId: newLinkedIds[0] || null,
            ...(newLinkedIds.length === 0 ? { purpose: '' } : {}),
            updatedAt: Timestamp.now(),
      ...
      // Include the project transaction delete in the combined batch
      combinedBatch.delete(projectTrxRef);
      await combinedBatch.commit();

      // Revert PRs after batch succeeds (情景 E) — must be post-commit, cannot be batched
    ...
    try {
      ...
      if (discrepancies.length === 0) {
        try {
          ...
          // Compensate: delete the reconciliation record AND restore the bank account balance
          // that phase-1 overwrote, so the account returns to a fully clean pre-reconciliation state.
          // Leaving the balance at statementBalance while the reconciliation doc is gone would
          // permanently misrepresent the account balance with no audit trail.
          try {
            const rollbackBatch = writeBatch(db);
            rollbackBatch.delete(reconciliationRef);
            rollbackBatch.update(doc(db, COLLECTIONS.BANK_ACCOUNTS, accountId), {
              balance: (account as any).balance ?? 0,
              lastReconciled: (account as any).lastReconciled ?? null,
              updatedAt: Timestamp.now(),
            });
