...
exports.gamificationFunctions = exports.calculatePointsFromRules = exports.updateAchievementProgress = exports.checkBadgeAwards = void 0;
...
exports.checkBadgeAwards = functions
    ...
    if (SELF_TRIGGERING_COLLECTIONS.includes(collectionName))
        return null;
    // F04 FIX step 1: skip collections that cannot affect any badge criteria
    if (!BADGE_RELEVANT_COLLECTIONS.includes(collectionName))
        return null;
    ...
    async function checkSingleBadge(badgeDoc) {
...
async function evaluateBadgeCriteria(criteria, memberId) {
...
async function checkEventAttendanceCriteria(criteria, memberId) {
...
async function checkProjectCompletionCriteria(criteria, memberId) {
...
async function checkPointsThresholdCriteria(criteria, memberId) {
...
async function checkCustomCriteria(criteria, memberId) {
    // Custom criteria would be implemented based on specific requirements
    // For now, return false
    return { eligible: false, progress: null };
}
// Function to update achievement progress
exports.updateAchievementProgress = functions
    .runWith({ timeoutSeconds: 300, memory: '512MB' })
    .firestore
    .document('{collection}/{documentId}')
    .onWrite(async (change, context) => {
    ...
    for (const achievementDoc of achievementsSnapshot.docs) {
        ...
        try {
            ...
            await db.runTransaction(async (txn) => {
                ...
                newMilestones = progress.completedMilestones.filter((m) => !currentData.completedMilestones.includes(m));
            ...
            for (const milestoneLevel of newMilestones) {
                const milestone = achievement.milestones.find((m) => m.level === milestoneLevel);
                if (milestone) {
...
exports.calculatePointsFromRules = functions
    .runWith({ timeoutSeconds: 300, memory: '512MB' })
...
exports.gamificationFunctions = {
    checkBadgeAwards: exports.checkBadgeAwards,
    updateAchievementProgress: exports.updateAchievementProgress,
    calculatePointsFromRules: exports.calculatePointsFromRules
};
