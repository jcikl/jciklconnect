...
async function calculateAchievementProgress(achievement: any, memberId: string): Promise<{ current: number; completedMilestones: string[] }> {
  ...
  const criteriaType: string = achievement.criteria?.type || '';
  ...
  // --- milestone / points threshold ---
    ...
    const memberSnap = await db.collection('members').doc(memberId).get();
    current = memberSnap.exists ? (memberSnap.data()?.points || 0) : 0;
  }
  ...
  const milestones: any[] = Array.isArray(achievement.milestones) ? achievement.milestones : [];
