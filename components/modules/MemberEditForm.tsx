            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-x-6 md:gap-y-4">
                ...
                <div>
                  ...
                  <input name="email" type="email" value={formValues.email} onChange={(e) => { const filtered = e.target.value.replace(/[^a-zA-Z0-9@._+\-]/g, ''); handleChange('email', filtered); }} onKeyDown={(e) => { if (!e.ctrlKey && !e.metaKey && !e.altKey && !['Backspace','Delete','Tab','Enter','ArrowLeft','ArrowRight','ArrowUp','ArrowDown','Home','End'].includes(e.key) && !/^[a-zA-Z0-9@._+\-]$/.test(e.key)) e.preventDefault(); }} onPaste={(e) => { e.preventDefault(); document.execCommand('insertText', false, e.clipboardData.getData('text').replace(/[^a-zA-Z0-9@._+\-]/g, '')); }} onCompositionEnd={(e) => { const input = e.currentTarget; const filtered = input.value.replace(/[^a-zA-Z0-9@._+\-]/g, ''); if (filtered !== input.value) { const setter = Object.getOwnPropertyDescriptor(window.HTMLInputElement.prototype, 'value')?.set; setter?.call(input, filtered); input.dispatchEvent(new Event('input', { bubbles: true })); } }} required className="w-full rounded-lg border border-slate-300 px-3 py-1.5 text-sm focus:border-jci-blue focus:ring-2 focus:ring-jci-blue/20" />
                </div>
